...
;(function () {
  ...
  HLP.packSHORTVAL = function (SHORTVAL) {
    return HLP.packBytes(SHORTVAL, DTS.SHORTVAL)
  }

  HLP.unpackSHORTVAL = function (SHORTVAL) {
    SHORTVAL = HLP.toByteArray(SHORTVAL)
    return HLP.unpack(SHORTVAL)
  }

  HLP.packTLV = function (type, value) {
    return HLP.packSHORTVAL(type) + HLP.packSHORTVAL(value.length) + value
  }
  ...
  HLP.splitype = function splitype(arr, msg) {
    ...
    arr.forEach(function (a) {
      ...
      switch (a) {
        case 'PUBKEY':
          str = splitype(['SHORTVAL', 'MPI', 'MPI', 'MPI', 'MPI'], msg).join('')
          break
...
;(function () {
  ...
  DSA.parsePublic = function (str, priv) {
    var fields = ['SHORTVAL', 'MPI', 'MPI', 'MPI', 'MPI']
...
;(function () {
  ...
  Parse.parseMsg = function (otr, msg) {
    ...
    if (com === ':') {
      ...
      if (!otr['ALLOW_V' + HLP.unpackSHORTVAL(version)]) return { msg: msg }
...
;(function () {
  ...
  OTR.prototype.handleTLVs = function (tlvs, sessKeys) {
    var type, len, msg
    for (; tlvs.length; ) {
      type = HLP.unpackSHORTVAL(tlvs.substr(0, 2))
      len = HLP.unpackSHORTVAL(tlvs.substr(2, 2))
  ...
  OTR.prototype.sendFile = function (filename) {
    ...
    msg += HLP.packSHORTVAL(4 + l1name.length)  // length of value
