...
ZaController = function(appCtxt, container, iKeyName) {
    ...
    /**
     * The name of the current controller. This name is used as a key in
     * {@link #initToolbarMethods}, {@link #setViewMethods} and
     * {@link #initPopupMenuMethods} maps
     */
    ...
    this._contentView = null; // the view object associated with the current
                                // controller instance
...
/**
 * A map of funciton references. Functions in this map are called one after
 * another from {@link #_initToolbar} method. The functions are called on the
 * current instance of the controller. member of ZaController
 * 
 * @see #_initToolbar
 */
...
/**
 * A map of funciton references. Functions in this map are called one after
 * another from {@link #_initPopupMenu} method. The functions are called on the
 * current instance of the controller. member of ZaController
 * 
 * @see #_initPopupMenu
 */
...
/**
 * A map of funciton references. Functions in this map are called one after
 * another from {@link #_setView} method. The functions are called on the
 * current instance of the controller. member of ZaController
 * 
 * @see #_setView
 */
...
ZaController.helpURL = location.pathname + ZaUtil.HELP_URL + "administration_console_help.htm?locid="
...
ZaController.prototype.getProgressDialog = function() {
    if (!ZaApp.getInstance().dialogs["progressDialog"])
        ZaApp.getInstance().dialogs["progressDialog"] = new ZaXProgressDialog(this._appCtxt.getShell(), "300px",
                "300px");
    return ZaApp.getInstance().dialogs["progressDialog"];
}

ZaController.prototype.setDirty = function(isD) {
    // overwrite this method to disable toolbar buttons, for example, Save
    // button
}

ZaController.prototype.setCurrentView = function(view) {
...
ZaController.prototype.setEnabled = function(enable) {
    // abstract
    // throw new AjxException("This method is abstract",
    // AjxException.UNIMPLEMENTED_METHOD, "ZaController.prototype.setEnabled");
}

ZaController.prototype.popupErrorDialog = function(msg, ex, style) {
...
ZaController.prototype.popupMsgDialog = function(msg, noExecReset) {
    if (!noExecReset)
        this._execFrame = {
            func : null,
            args : null,
...
ZaController.prototype.popupWarningDialog = function(msg, noExecReset) {
    if (!noExecReset)
        this._execFrame = {
            func : null,
            args : null,
...
ZaController.prototype.getControllerForView = function(view) {
...
/**
 * @param nextViewCtrlr -
 *                the controller of the next view
 * @param func -
 *                the method to call on the nextViewCtrlr in order to navigate
 *                to the next view
 * @param params -
 *                arguments to pass to the method specified in func parameter
 *                Ovewrite this method in order to check if it is OK to leave
 *                the current view or perform any actions before a user
 *                navigates away from the view.
 */
ZaController.prototype.switchToNextView = function(nextViewCtrlr, func, params) {
...
/**
 * This method finds an array of function references in
 * {@link ZaController#setViewMethods} map and calls all the functions for the
 * array. {@link #_iKeyName} is used to lookup the array of function references
 * in the map.
 * 
 * @private
 */
ZaController.prototype._setView = function(entry, openInNewTab, skipRefresh) {
 ...
 * 
 * @param ev
 *                event object handles the Close button click. Returns to the
 *                previous view.
 */
ZaController.prototype.closeButtonListener = function(ev, noPopView, func, obj, params) {
...
/**
 * We do the whole schedule/execute thing to give the shell the opportunity to
 * popup its "busy" overlay so that user input is blocked. For example, if a
 * search takes a while to complete, we don't want the user's clicking on the
 * search button to cause it to re-execute repeatedly when the events arrive
 * from the UI. Since the action is executed via win.setTimeout(), it must be a
 * leaf action (scheduled actions are executed after the calling code returns to
 * the UI loop). You can't schedule something, and then have subsequent code
 * that depends on the scheduled action.
 * 
 * @private
 */
...
ZaController.prototype._handleException = function(ex, method, params, restartOnError, obj) {
    DBG.dumpObj(ex);
    if (ex.code
            && (ex.code == ZmCsfeException.SVC_AUTH_EXPIRED || ex.code == ZmCsfeException.SVC_AUTH_REQUIRED
                    || ex.code == ZmCsfeException.NO_AUTH_TOKEN || ex.code == ZmCsfeException.AUTH_TOKEN_CHANGED)) {
        try {
            ...
            this._loginDialog.registerCallback(this.loginCallback, this);
            /*
             * Sometimes, users will clear cookie manually, that will cause
             * security issue. see: bug 67427 But in the process of login, we
             * use this exception to popup login dialog if user doesn't login.
             * We shouldn't disable the username field in the first soap request
             * if an exception is thrown.
             */
            if ((!ZaZimbraAdmin.isFirstRequest && (ex.code == ZmCsfeException.NO_AUTH_TOKEN
                    || ex.code == ZmCsfeException.SVC_AUTH_REQUIRED || ex.code == ZmCsfeException.SVC_AUTH_EXPIRED))) {
        ...
        if (!ex.code) {
        ...
        } else if (ex.code == ZmCsfeException.NETWORK_ERROR || ex.code == AjxException.NETWORK_ERROR) {
            ...
            this.popupErrorDialog(ZaMsg.ERROR_TOO_MANY_ACCOUNTS, ex, true);
        ...
        } else if (ex.code == ZmCsfeException.CSFE_SVC_ERROR || ex.code == ZmCsfeException.SVC_FAILURE
...
ZaController.prototype._doAuth = function(username, password) {
    ZmCsfeCommand.noAuth = true;
    try {
        // hide login dialog
...
ZaController.prototype._hideLoginDialog = function(clear) {
 ...
 */
ZaController.prototype.authCallback = function(resp) {
    ...
    if (resp.isException && resp.isException()) {
        ...
        if (ex.code == ZmCsfeException.ACCT_AUTH_FAILED || ex.code == ZmCsfeException.SVC_PERM_DENIED
            ...
            this._showLoginDialog(true);
            var msg = ex.code == ZmCsfeException.ACCT_PASS_RECENTLY_USED ? ZaMsg.errorPassRecentlyUsed
...
ZaController.prototype.loginCallback = function(uname, password) {
    // this._schedule(this._doAuth, {username: uname, password: password});
    this._doAuth(uname, password);
}

ZaController.prototype.changePwdCallback = function(uname, oldPass, newPass, conPass) {
    ...
    try {
        ...
        resp = ZaController.changePwdCommand.invoke({
            soapDoc : soapDoc,
            noAuthToken : true,
        ...
        if (ex.code == ZmCsfeException.ACCT_PASS_RECENTLY_USED || ex.code == ZmCsfeException.ACCT_PASS_CHANGE_TOO_SOON) {
            var msg = ex.code == ZmCsfeException.ACCT_PASS_RECENTLY_USED ? ZaMsg.errorPassRecentlyUsed
                    : (ZaMsg.errorPassChangeTooSoon);
            this._loginDialog.setError(msg);
            this._loginDialog.setFocus(null);
...
/**
 * This method finds an array of function references in
 * {@link ZaController#initToolbarMethods} map and calls all the functions for
 * the array. {@link #_iKeyName} is used to lookup the array of function
...
/**
 * This method finds an array of function references in
 * {@link ZaController#initPopupMenuMethods} map and calls all the functions for
 * the array. {@link #_iKeyName} is used to lookup the array of function
 * references in the map.
 * 
 * @private
 ...
 * @param listener
 */
ZaController.prototype.addCreationListener = function(listener) {
 ...
 * @param listener
 */
ZaController.prototype.removeCreationListener = function(listener) {
 ...
 * @param listener
 */
ZaController.prototype.addRemovalListener = function(listener) {
 ...
 * @param listener
 */
ZaController.prototype.removeRemovalListener = function(listener) {
    this._evtMgr.removeListener(ZaEvent.E_REMOVE, listener);
}

/**
 * member of ZaXFormViewController Adds listener to modifications in the
 * contained ZaAccount
 * 
 * @param listener
 */
ZaController.prototype.addChangeListener = function(listener) {
    this._evtMgr.addListener(ZaEvent.E_MODIFY, listener);
}

/**
 * member of ZaXFormViewController Removes listener to modifications in the
 * controlled ZaAccount
 * 
 * @param listener
 */
ZaController.prototype.removeChangeListener = function(listener) {
 ...
 * @param listener
 */
ZaController.prototype.addSearchListener = function(listener) {
 ...
 * @param listener
 */
ZaController.prototype.removeSearchListener = function(listener) {
    this._evtMgr.removeListener(ZaEvent.E_SEARCH, listener);
}

/**
 * member of ZaXFormViewController Private method that notifies listeners that a
 * new object is created
 * 
 * @param details
 */
ZaController.prototype.fireCreationEvent = function(details) {
    try {
...
/**
 * Private method that notifies listeners to that the controlled ZaAccount is
 * (are) removed
 * 
 * @param details
 */
ZaController.prototype.fireRemovalEvent = function(details) {
    try {
...
/**
 * member of ZaXFormViewController Method that notifies listeners to that the
 * controlled object is changed
 * 
 * @param details
 *                {String}
 */
ZaController.prototype.fireChangeEvent = function(details) {
    try {
 ...
 * @param details
 */
ZaController.prototype.fireSearchEvent = function(details) {
    try {
...
ZaController.showTooltip = function(event, item) {
...
ZaController.hideTooltip = function(event) {
    var shell = DwtShell.getShell(window);
    var tooltip = shell.getToolTip();
    tooltip.popdown();
}

ZaController.prototype.selectExistingTabByItemId = function(itemId, tabConstructor) {
    return false;
}
...
ZaController.prototype.changeActionsState = function() {
    ...
    if (this._actionMenu && this._popupOperations) {
        for ( var i in this._popupOperations) {
            if (this._popupOperations[i] instanceof ZaOperation
                    && !AjxUtil.isEmpty(this._actionMenu.getMenuItem(this._popupOperations[i].id))) {
    ...
    if (isCurrentController && this._popupOperations && settingMenu) {
        for ( var i in this._popupOperations) {
            if (this._popupOperations[i] instanceof ZaOperation
                    && !AjxUtil.isEmpty(settingMenu.getMenuItem(this._popupOperations[i].id))) {
...
ZaController.prototype._showAccountsView = function(defaultType, ev, filterQuery) {
    ...
    if (defaultType != ZaItem.ALIAS) { // alias uid has no domain name, we
                                        // shouldn't add a domain name filter.
                                        // See bug 46626, 44799 & 4704
        if (!ZaSettings.HAVE_MORE_DOMAINS
    ...
    if (ZaApp.getInstance().getCurrentController()) {
        ZaApp.getInstance().getCurrentController().switchToNextView(acctListController,
                ZaAccountListController.prototype.show, [ true, ev.refresh ]);
    } else {
        acctListController.show(true);
    }
};

ZaController.prototype.cancelBusyOverlay = function(params) {
